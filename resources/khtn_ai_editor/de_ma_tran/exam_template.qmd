...
def _as_positive_int(raw_value, name: str) -> int:
    try:
        value = int(raw_value)
    except (TypeError, ValueError) as exc:
        raise ValueError(f"Tham số {name} phải là số nguyên.") from exc
    if value <= 0:
        raise ValueError(f"Tham số {name} phải lớn hơn 0.")
    return value


seed_value = _as_positive_int(params["seed"], "seed")
num_mcq = _as_positive_int(params["num_mcq"], "num_mcq")
num_essay = _as_positive_int(params["num_essay"], "num_essay")
