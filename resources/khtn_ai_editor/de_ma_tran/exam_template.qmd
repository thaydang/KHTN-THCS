...
def pick_items(bank: List[Dict[str, str]], count: int) -> List[Dict[str, str]]:
    validated_count = _as_positive_int(count, "số lượng câu hỏi", allow_zero=True)
    if validated_count > len(bank):
        raise ValueError(
            f"Không đủ câu hỏi trong ngân hàng (cần {validated_count}, có {len(bank)})."
        )
    return rng.sample(bank, k=validated_count)
