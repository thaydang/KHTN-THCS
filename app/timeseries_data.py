...
class TimeseriesData:
    ...
    def validate(self) -> tuple[bool, Optional[str]]:
        ...
        if len(self.timeseries) < 5:
            return False, f"Timeseries must have at least 5 data points, got {len(self.timeseries)}"
        
        # Check all time values are non-negative
        for i, point in enumerate(self.timeseries):
            if point.time_s < 0:
                return False, f"Time value at index {i} must be non-negative, got {point.time_s}"
        ...
        if self.metadata.sampling_rate_hz <= 0:
            return False, f"Metadata.sampling_rate_hz must be positive, got {self.metadata.sampling_rate_hz}"
    ...
    def from_dict(cls, data: Dict[str, Any]) -> TimeseriesData:
        ...
        timeseries = [
            TimeseriesDataPoint(
                time_s=point.get("time_s", 0.0),
                temp_C=point.get("temp_C", 0.0)
            )
